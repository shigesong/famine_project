...
```{r, message=F, echo=F, warning=FALSE, tidy=F, results='hide'}
library(ggplot2)
library(ggthemes)
library(Zelig)
data(turnout)
...
```{r, results='asis', message=FALSE, prompt=FALSE}
library(reshape2)
library(pander)
...
Here is another way to do it using the "htmlTable" function in the *[Gmisc](http://gforge.se/gmisc/)* package (not yet in CRAN):

```{r, results='asis', message=FALSE, prompt=FALSE, warning=FALSE}
library(Gmisc)
